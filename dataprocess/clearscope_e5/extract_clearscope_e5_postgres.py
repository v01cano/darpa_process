"""
ClearScope E5 数据提取脚本（PostgreSQL 版）

与 extract_clearscope_e5.py 提取逻辑完全一致，结果直接写入 PostgreSQL。
表结构与其他四个数据集统一。

数据量较大（~24GB 双 bin.json）→ 第 2 遍边写入采用 50k 批量提交。

用法：
  psql -U postgres -f init_database.sql
  python extract_clearscope_e5_postgres.py --input_dir /mnt/disk/darpa/clearscope_e5
"""
...
from extract_clearscope_e5 import (
...
def extract_and_store(input_files, cur, conn):
    """两遍：1) 收集实体写节点表；2) 提取边批量写 event_table。"""
    uuid2name = {}
    uuid2index = {}
    netflow_info = {}
    ...
    print(f"  第 1 遍完成: {loaded_line:,} 行, {time.time()-begin:.1f}s")
    ...
    for vidx, vp in enumerate(input_files):
        ...
        with open(vp, 'r') as fin:
            for line in fin:
                loaded_line += 1
                if loaded_line % 1000000 == 0:
                    print(f"      已扫描 {loaded_line:,} 行... 累计写入边 {sum(edge_type_count.values()):,}")
                try:
                    record = json.loads(line)['datum']
                except Exception:
                    continue
    ...
    print(f"  第 2 遍完成: {loaded_line:,} 行, {time.time()-begin:.1f}s")
...
def main(args):
    print("=" * 60)
    print("ClearScope E5 数据提取（PostgreSQL）")
    print("=" * 60)
    print(f"\n输入目录: {args.input_dir}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="ClearScope E5 Data Extractor (PostgreSQL)")
    parser.add_argument("--input_dir", type=str, default="/mnt/disk/darpa/clearscope_e5")
    parser.add_argument("--db_name", type=str, default="clearscope_e5")
