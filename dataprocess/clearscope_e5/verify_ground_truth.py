"""
ClearScope E5 Ground Truth 验证脚本

参照 fivedirections_e3/verify_ground_truth.py，适配 ClearScope E5：
  - 没有 thread_to_process 映射（ClearScope 无 SUBJECT_THREAD）
  - 边输出是分片 edges_part_*.pkl
  - UUID 强制小写匹配（避免 FiveDirections 那种坑）

流程：
1. 读取 uuid2name.pkl
2. 读取 ground truth CSV（如果有）
3. 流式扫描 edges_part_*.pkl，只保留攻击节点相关边
4. 输出 UUID 命中率 + 攻击图 + 时间序列
"""
...
def verify_one_attack(attack_name, gt_nodes, u2n, src_index, dst_index):
    ...
    for u, gt_type, gt_name, our_type, our_name in found:
        match = "✓" if our_type == gt_type else "✗类型"
        our_s = str(our_name)[:50] if our_name else '?'
        gt_s = str(gt_name)[:50]
        print(f"    [{our_type:8s}] {match} 我们={our_s:50s} GT={gt_s}")
    ...
    if unique_edges:
        ...
        print(f"\n--- 3. 攻击链事件序列（最多 200 条） ---")
        for i, edge in enumerate(unique_edges[:200]):
            ts, etype, su, st, sn, du, dt, dn, _ = edge
            sl = f"{sn}({st})"[:42] if sn else f"?({st})"
            dl = f"{dn}({dt})"[:42] if dn else f"?({dt})"
            print(f"    [{ts}] {sl:42s} --{etype:18s}--> {dl}")
