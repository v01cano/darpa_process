...
参照 cadets_e3/analyze_all_entity_types.py 和 clearscope_e5/analyze_all_entity_types.py。

目的：
  1. 每种 Event.type 引用什么实体（subject / predicateObject 的类型分布）
  2. 哪些 Event 携带 predicateObjectPath / cmdLine / exec
  3. 是否存在 IpcObject / SrcSinkObject / MemoryObject / RegistryKeyObject 等
...
def analyze(filepaths, max_lines=None):
    ...
    print(f"  Pass1 完成，建立 {len(uuid_to_type):,} 个 uuid→类型")
