"""
CADETS E5 数据提取脚本（PostgreSQL 版）

与 extract_cadets_e5.py 提取逻辑完全一致，结果直接写入 PostgreSQL。
表结构与其他六个数据集统一。
...
def extract_and_store(input_files, cur, conn):
    """两遍：1) 实体收集 + Event.exec/path 更新；2) 边提取 + 批量写入。"""
    uuid2name = {}
    uuid_cmdline = {}
    netflow_info = {}
    uuid2index = {}
...
def main(args):
    print("=" * 60)
    print("CADETS E5 数据提取（PostgreSQL）")
    print("=" * 60)
    print(f"\n输入目录: {args.input_dir}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="CADETS E5 Data Extractor (PostgreSQL)")
    parser.add_argument("--input_dir", type=str,
                        default="/mnt/disk/darpa/cch_refine/cadets_e5_json")
    parser.add_argument("--db_name", type=str, default="cadets_e5")
