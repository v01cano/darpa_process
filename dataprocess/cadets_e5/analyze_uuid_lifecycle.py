"""
CADETS E5 UUID 生命周期分析

参照 trace_e3/analyze_uuid_lifecycle.py 和 clearscope_e5/analyze_uuid_lifecycle.py。

重点验证：
  1. Subject 类型分布（PROCESS / THREAD / UNIT 等）
  2. UUID 大小写
  3. parentSubject 链
  4. FORK / CLONE / EXECUTE 数量与样本
     - CADETS E3：fork+exec 分离模型
     - 本数据集是否仍是该模型？
  5. EXECUTE 后是否产生新 UUID（同 TRACE E3 模式）？
"""
...
def analyze(filepaths, max_lines=None):
    ...
    # FORK/EXECUTE 是否会让 src 改 UUID？观察 EXECUTE 前后 subject UUID
    subject_first_seen = {}      # subject_uuid → first event timestamp
    ...
    print(f"  Distinct subject UUIDs in EXECUTE events: {len(execute_subject_uuids):,}")
    print(f"  → 如果 EXECUTE 数量较大且 UUID 是新出现的 → TRACE 风格（exec 造新 UUID）")
    print(f"  → 如果 EXECUTE 数量大但 UUID 与 fork 中的相同 → 标准 fork+exec 模型")
    ...
    # 不一致检测：EXECUTE 中的 subject UUID 是否在 Subject 记录里定义过
    if execute_subject_uuids:
        ...
        print(f"    其中是已定义 PROCESS: {defined_in_subj:,}")
