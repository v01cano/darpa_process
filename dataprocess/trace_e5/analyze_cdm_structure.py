"""
TRACE E5 CDM20 数据结构分析

参照 trace_e3 / theia_e5 / cadets_e5。

TRACE E3 vs TRACE E5 待验证：
  - CDM 命名空间: cdm18 → cdm20？
  - SUBJECT_UNIT 是否仍存在（E3 大量，需要丢弃）
  - Subject.path / cmdLine 填充率
  - Subject.cmdLine 是 str 还是 dict
  - FileObject.filename 填充率
  - NetFlow 是 str 还是 dict
  - UUID 大小写
  - EVENT_EXECUTE 是否仍造新 UUID（TRACE 特色）
"""
...
def analyze_files(filepaths, max_lines=None):
    ...
    print(f"\n{'='*80}\n七、E5 vs TRACE E3 关键差异自检\n{'='*80}")
    print(f"  Schema namespace            : {dict(schema_namespace_counter)}")
    unit_cnt = subject_type_counter.get('SUBJECT_UNIT', 0)
    print(f"  SUBJECT_UNIT 是否存在       : {unit_cnt:,}  (TRACE E3 大量，需丢弃)")
    print(f"  Subject.props.path 填充     : {subject_has_props_path/max(subject_count,1)*100:.1f}% (TRACE E3 高)")
    print(f"  Subject.cmdLine 填充        : {subject_has_cmdline/max(subject_count,1)*100:.1f}%")
    print(f"    其中 str / dict           : {subject_cmdline_is_str:,} / {subject_cmdline_is_dict:,}")
    print(f"  FileObject filename 填充    : {file_has_filename/max(file_count,1)*100:.1f}% (TRACE E3 高)")
    ...
    print(f"  EXECUTE 事件数              : {event_type_counter.get('EVENT_EXECUTE', 0):,}")
    print(f"  FORK + CLONE 事件数         : {event_type_counter.get('EVENT_FORK', 0) + event_type_counter.get('EVENT_CLONE', 0):,}")
