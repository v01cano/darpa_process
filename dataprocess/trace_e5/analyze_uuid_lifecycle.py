"""
TRACE E5 UUID 生命周期分析

参照 trace_e3 / theia_e5 / cadets_e5。

重点验证（与 TRACE E3 对比）：
  1. Subject 类型分布
     - TRACE E3: SUBJECT_PROCESS + 大量 SUBJECT_UNIT (UNIT 丢弃)
     - E5 是否仍有 UNIT？
  2. UUID 大小写
  3. parentSubject 链
  4. EVENT_EXECUTE 是否仍造新 UUID（TRACE 特色 vs 其他数据集的 fork+exec 分离）
     关键观察：EXECUTE 中 subject UUID 是否已在 Subject 表中定义
     - TRACE E3: 大量未定义（执行后造新 UUID）
     - CADETS/THEIA E5: 100% 已定义（标准 fork+exec）
  5. FORK / CLONE / EXECUTE 数量
"""
...
def analyze(filepaths, max_lines=None):
    ...
    print(f"  Distinct predObj UUIDs in EXECUTE: {len(execute_predobj_uuids):,}")

    print(f"\n=== TRACE 关键: EXECUTE 是否造新 UUID? ===")
    if execute_subject_uuids:
        defined = sum(1 for u in execute_subject_uuids if u in process_uuids)
        undefined = len(execute_subject_uuids) - defined
        print(f"  EXECUTE subject UUID 已定义为 PROCESS: {defined}/{len(execute_subject_uuids)} ({defined/len(execute_subject_uuids)*100:.1f}%)")
        print(f"  EXECUTE subject UUID 未定义:           {undefined}/{len(execute_subject_uuids)} ({undefined/len(execute_subject_uuids)*100:.1f}%)")
        print(f"\n  → 若大量未定义 → TRACE 风格（exec 造新 UUID，需保留 EXECUTE 边作身份切换）")
        print(f"  → 若 100% 已定义 → 标准 fork+exec 分离（同 THEIA E5 / CADETS E5）")
    if execute_predobj_uuids:
        in_process = sum(1 for u in execute_predobj_uuids if u in process_uuids)
        print(f"\n  EXECUTE predObj UUID 是已定义 PROCESS:  {in_process}/{len(execute_predobj_uuids)}")
        print(f"  (TRACE E3 中 predObj 是新进程 UUID)")
    ...
    for su, pu, pop in execute_records[:10]:
        ...
        defined_su = '✓' if su in process_uuids else '✗未定义'
        defined_pu = '✓' if pu in process_uuids else '?'
        print(f"  subject={su}[{defined_su}]  predObj={pu}[{defined_pu}]  path={path}")
