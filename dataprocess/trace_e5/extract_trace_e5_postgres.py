"""
TRACE E5 数据提取（PostgreSQL 版）。复用 extract_trace_e5 的 Pass1。
"""
...
def pass2_to_db(input_files, uuid2name, uuid2index, uuid_cmdline, cur, conn):
    print("\n  Pass 2: 边提取 + 写入 ...")
    begin = time.time()
    loaded_line = 0
    datalist = []
...
def main(args):
    print("=" * 60)
    print("TRACE E5 数据提取（PostgreSQL）")
    print("=" * 60)
    print(f"\n输入目录: {args.input_dir}")
    ...
    pass2_to_db(input_files, uuid2name, uuid2index, uuid_cmdline, cur, conn)
    cur.close()
    conn.close()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="TRACE E5 Data Extractor (PostgreSQL)")
    parser.add_argument("--input_dir", type=str, default="/mnt/disk1/darpa_e5/trace")
    parser.add_argument("--db_name", type=str, default="trace_e5")
