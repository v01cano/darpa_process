"""
TRACE E3 CDM数据结构分析脚本

TRACE 的已知特征（来自 CAPTAIN trace_parser.py）：
  - CDM18 格式（同 CADETS/THEIA/ClearScope）
  - Subject.properties.map.name 有进程名（类似 CADETS 的字段名）
  - Subject.cmdLine 有值（dict或string）
  - Subject.properties.map.ppid 有值
  - 无 Event.exec 字段（类似 THEIA）
  - FileObject 路径在 baseObject.properties.map.path
  - EVENT_EXECUTE → CAPTAIN 映射为 'update_process'
  - 同时有 EVENT_FORK 和 EVENT_CLONE

用法：
  python analyze_cdm_structure.py /mnt/disk/darpa/trace_e3/ta1-trace-e3-official.json
"""
...
def analyze_file(filepath):
        ...
        for line in fin:
            ...
            if rtype == 'Subject':
                ...
                if props_map.get('ppid') is not None:
                    subject_has_ppid += 1
                if datum.get('cid') is not None:
                    subject_has_cid += 1
                if datum.get('parentSubject') is not None:
                ...
                base_props = base.get('properties', {}).get('map', {}) if isinstance(base.get('properties'), dict) else {}
                ...
                if isinstance(datum.get('predicateObject2'), dict): event_has_predicate_object2 += 1
                if isinstance(datum.get('predicateObjectPath'), dict):
                    event_has_predicate_object_path += 1
                    event_type_has_pred_obj_path[etype] += 1
                if isinstance(datum.get('predicateObject2Path'), dict): event_has_predicate_object2_path += 1
    ...
    if subject_count > 0:
        sc = subject_count
        print(f"\n  --- 字段填充率 ---")
        print(f"  properties.map.name : {subject_has_props_name:>8,} / {sc:,}  ({subject_has_props_name/sc*100:5.1f}%)")
        print(f"  properties.map.path : {subject_has_props_path:>8,} / {sc:,}  ({subject_has_props_path/sc*100:5.1f}%)")
        print(f"  properties.map.tgid : {subject_has_tgid:>8,} / {sc:,}  ({subject_has_tgid/sc*100:5.1f}%)")
        print(f"  properties.map.ppid : {subject_has_ppid:>8,} / {sc:,}  ({subject_has_ppid/sc*100:5.1f}%)")
    ...
    print(f"\n  name样例: {subject_name_samples[:5]}")
    print(f"  path样例: {subject_path_samples[:5]}")
    ...
    print(f"  filename样例: {file_filename_samples[:5]}")
    ...
    if net_count > 0:
        print(f"  localAddress 非null: {net_has_local_addr:,}  类型: {dict(net_local_type)}")
        print(f"  remoteAddress非null: {net_has_remote_addr:,}  类型: {dict(net_remote_type)}")
    ...
    if event_count > 0:
        ...
        print(f"  predicateObject      : {event_has_predicate_object:>10,} / {ec:,}  ({event_has_predicate_object/ec*100:5.1f}%)")
        print(f"  predicateObject2     : {event_has_predicate_object2:>10,} / {ec:,}  ({event_has_predicate_object2/ec*100:5.1f}%)")
        print(f"  predicateObjectPath  : {event_has_predicate_object_path:>10,} / {ec:,}  ({event_has_predicate_object_path/ec*100:5.1f}%)")
        print(f"  predicateObject2Path : {event_has_predicate_object2_path:>10,} / {ec:,}  ({event_has_predicate_object2_path/ec*100:5.1f}%)")
        print(f"  name字段             : {event_has_name:>10,} / {ec:,}  ({event_has_name/ec*100:5.1f}%)")
        print(f"\n  --- properties.map中exec/cmdLine ---")
        print(f"  有exec字段的Event    : {event_has_exec:>10,} / {ec:,}  ({event_has_exec/ec*100:5.1f}%)")
        print(f"  有cmdLine字段的Event : {event_has_cmdline:>10,} / {ec:,}  ({event_has_cmdline/ec*100:5.1f}%)")
    ...
    print(f"\n{'='*80}")
    print("七、与其他数据集的关键差异对比")
    print(f"{'='*80}")
    print(f"  Subject.properties.map.name 填充率:  {subject_has_props_name/max(subject_count,1)*100:.1f}% (CADETS=0%, THEIA=0%, ClearScope=0%)")
    print(f"  Subject.properties.map.path 填充率:  {subject_has_props_path/max(subject_count,1)*100:.1f}% (CADETS=0%, THEIA=99.3%, ClearScope=0%)")
    print(f"  Subject.cmdLine 填充率:              {subject_has_cmdline/max(subject_count,1)*100:.1f}% (CADETS=0%, THEIA=99.3%, ClearScope=100%)")
    print(f"  FileObject path 填充率:              {file_has_path/max(file_count,1)*100:.1f}% (CADETS=0%, THEIA=0%, ClearScope=100%)")
    print(f"  FileObject filename 填充率:          {file_has_filename/max(file_count,1)*100:.1f}% (CADETS=0%, THEIA=97.4%, ClearScope=0%)")
    print(f"  Event.exec 填充率:                   {event_has_exec/max(event_count,1)*100:.1f}% (CADETS=99.9%, THEIA=0%, ClearScope=0%)")
    print(f"  Event.cmdLine 填充率:                {event_has_cmdline/max(event_count,1)*100:.1f}% (CADETS=0.5%, THEIA=0.1%, ClearScope=0%)")
    print(f"  Event.predicateObjectPath 填充率:    {event_has_predicate_object_path/max(event_count,1)*100:.1f}% (CADETS=44.2%, THEIA=0%, ClearScope=0.9%)")


if __name__ == '__main__':
    filepath = sys.argv[1] if len(sys.argv) > 1 else "/mnt/disk/darpa/trace_e3/ta1-trace-e3-official.json"
