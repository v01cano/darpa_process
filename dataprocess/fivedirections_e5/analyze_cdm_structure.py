"""
FiveDirections E5 CDM20 数据结构分析

参照 fivedirections_e3 / cadets_e5 / theia_e5 / clearscope_e5。

FD E5 vs FD E3 待验证：
  - CDM 命名空间: cdm18 → cdm20？
  - SUBJECT_PROCESS / SUBJECT_THREAD 比例（E3 5% 是 THREAD）
  - RegistryKeyObject 是否还存在
  - Subject.cmdLine 是 str / dict
  - NetFlow 地址格式
  - UUID 大小写（E3 混合大写）
  - 新增 Host / Principal / MemoryObject 等

用法:
  python analyze_cdm_structure.py [<file>...]
"""
...
def analyze_files(filepaths, max_lines=None):
    ...
    for fp in filepaths:
        ...
        with open(fp, 'r') as fin:
            for line in fin:
                ...
                elif record_type == 'RegistryKeyObject':
                    reg_count += 1
                    k = body.get('key')
                    if k is not None:
                        reg_has_key += 1
                        if len(reg_key_samples) < 10:
                            reg_key_samples.append(k if isinstance(k, str) else str(k)[:120])
    ...
    if subject_count > 0:
        ...
        print(f"  cid                 : {subject_has_cid:>8,} / {subject_count:,}  ({subject_has_cid/subject_count*100:5.1f}%)")
        print(f"  cmdLine             : {subject_has_cmdline:>8,} / {subject_count:,}  ({subject_has_cmdline/subject_count*100:5.1f}%)")
        print(f"    str: {subject_cmdline_is_str:,}  dict: {subject_cmdline_is_dict:,}  null: {subject_cmdline_is_null:,}")
        print(f"  parentSubject       : {subject_has_parent:>8,} / {subject_count:,}  ({subject_has_parent/subject_count*100:5.1f}%)")
    print(f"\n  Subject.properties.map 所有 key: {dict(subject_properties_keys)}")
    print(f"  Subject 顶层 key:")
    for k, cnt in subject_toplevel_keys.most_common():
        print(f"    {k:30s} {cnt:>10,}")
    print(f"  path 样例: {subject_path_samples[:5]}")
    ...
    if file_count > 0:
        print(f"  filename : {file_has_filename:>8,} / {file_count:,}  ({file_has_filename/file_count*100:5.1f}%)")
        print(f"  path     : {file_has_path:>8,} / {file_count:,}  ({file_has_path/file_count*100:5.1f}%)")
        print(f"  name     : {file_has_name:>8,} / {file_count:,}  ({file_has_name/file_count*100:5.1f}%)")
    print(f"\n  baseObject.props.map 所有 key:")
    for k, cnt in file_base_props_keys.most_common():
        print(f"    {k:30s} {cnt:>10,}")
    print(f"  filename 样例: {file_filename_samples[:5]}")
    print(f"  path 样例: {file_path_samples[:5]}")

    print(f"\n{'='*80}\n四、NetFlowObject 分析\n{'='*80}")
    print(f"  NetFlowObject总数: {net_count:,}")
    if net_count > 0:
        print(f"  localAddress  非null: {net_has_local_addr:>8,}  类型: {dict(net_local_addr_type)}")
        print(f"  remoteAddress 非null: {net_has_remote_addr:>8,}  类型: {dict(net_remote_addr_type)}")
        print(f"  样例: {net_samples[:3]}")

    print(f"\n{'='*80}\n四.B、RegistryKeyObject 分析（FD 独有）\n{'='*80}")
    print(f"  RegistryKeyObject总数: {reg_count:,}")
    if reg_count > 0:
        print(f"  有 key 的: {reg_has_key:>8,} ({reg_has_key/reg_count*100:5.1f}%)")
        print(f"  有 value 的: {reg_has_value:>8,}")
        print(f"  key 样例:")
        for k in reg_key_samples[:5]:
            print(f"    {k}")
    ...
    if event_count > 0:
        ...
        print(f"  name 字段            : {event_has_name:>10,} / {event_count:,}")
    ...
    print(f"\n{'='*80}\n七、E5 vs FD E3 关键差异自检\n{'='*80}")
    print(f"  Schema namespace            : {dict(schema_namespace_counter)}")
    thread_cnt = subject_type_counter.get('SUBJECT_THREAD', 0)
    print(f"  SUBJECT_THREAD 是否存在     : {thread_cnt:,}  (FD E3 大量)")
    print(f"  RegistryKeyObject 是否存在  : {reg_count:,}  (FD E3 大量)")
    print(f"  Subject.cmdLine dict 比例   : {subject_cmdline_is_dict/max(subject_count,1)*100:.1f}%")
    print(f"  Subject.props.path 填充     : {subject_has_props_path/max(subject_count,1)*100:.1f}%")
    print(f"  Event.predicateObjectPath   : {event_has_predicate_object_path/max(event_count,1)*100:.1f}%  (FD E3 高)")
    has_loadlib = event_type_counter.get('EVENT_LOADLIBRARY', 0)
    has_createthread = event_type_counter.get('EVENT_CREATE_THREAD', 0)
    print(f"  EVENT_LOADLIBRARY           : {has_loadlib:,}")
    print(f"  EVENT_CREATE_THREAD         : {has_createthread:,}")
