"""
THEIA E5 CDM20 数据结构分析脚本

参照 cadets_e5/clearscope_e5 的版本。

THEIA E3 vs THEIA E5 待验证：
  - CDM 命名空间: cdm18 → cdm20？
  - Subject.path / Subject.cmdLine 填充率（E3 为 99.3%）
  - FileObject.filename 填充率（E3 为 97.4%）
  - Subject.cmdLine 是 str 还是 dict
  - NetFlow 地址是直接 string 还是 dict
  - UUID 大小写
  - 是否还需要"两遍 + EXECUTE 更新 dst" 策略
...
def analyze_files(filepaths, max_lines=None):
    ...
    print(f"\n{'='*80}\n七、E5 vs THEIA E3 关键差异自检\n{'='*80}")
    print(f"  Subject.props.path 填充     : {subject_has_props_path/max(subject_count,1)*100:.1f}% (E3 THEIA=99.3)")
    print(f"  Subject.cmdLine 填充        : {subject_has_cmdline/max(subject_count,1)*100:.1f}% (E3 THEIA=99.3)")
    print(f"    其中 str / dict           : {subject_cmdline_is_str:,} / {subject_cmdline_is_dict:,}")
    print(f"  FileObject filename 填充    : {file_has_filename/max(file_count,1)*100:.1f}% (E3 THEIA=97.4)")
    print(f"  FileObject path 填充        : {file_has_path/max(file_count,1)*100:.1f}% (E3 THEIA=0)")
    print(f"  Event.exec 填充             : {event_has_exec/max(event_count,1)*100:.1f}% (E3 THEIA=0)")
    print(f"  Event.predicateObjectPath   : {event_has_predicate_object_path/max(event_count,1)*100:.1f}% (E3 THEIA=0)")
